...
export default function Lobby({ onJoin }) {
  ...
  const createRoom = () => socket.emit('room:create',{ name,maxPlayers:5 },(room)=>onJoin(room,socket));
  const joinRoom = () => socket.emit('room:join',{ code,name },(room)=>onJoin(room,socket));
  ...
  return (
    <div className="p-4">
      ...
      {rooms.map(r=>(<div key={r.code} className="border p-2 my-1">{r.code} - {r.players.length} players - {r.status}</div>))}
    </div>
  );
}
